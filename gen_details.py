#!/usr/bin/env python3
"""Generate a static detail page per local-LLM tool (p/<slug>/index.html) from data.json.
...
SITE_URL = "https://local-llm-index.vercel.app"
...
def page(it: dict, related: list[dict]) -> str:
    title = f"{it['full_name']} — {it['category']} | The Local LLM Index"
    desc = (it["description"] or f"{it['full_name']}, a {it['category']} tool on The Local LLM Index.")[:300]
    url = f"{SITE_URL}/p/{it['slug']}/"
    topics = "".join(f'<span class="topic">{esc(t)}</span>' for t in (it.get("topics") or [])[:12])
    rel = "".join(
        f'<a class="card in" href="/p/{esc(r["slug"])}/"><div class="card-head"><span class="cat">{esc(r["category"])}</span>'
        f'<span class="rk">#{r["rank"]}</span></div><div class="card-body"><div class="name disp">{esc(r["name"])}</div>'
    ...
    ld = {"@context": "https://schema.org", "@type": "SoftwareSourceCode", "name": it["full_name"],
          ...
          "programmingLanguage": it.get("language") or "C++",
          "author": {"@type": "Person", "name": it["owner"]}, "license": it.get("license") or ""}
    crumb = {"@context": "https://schema.org", "@type": "BreadcrumbList", "itemListElement": [
        {"@type": "ListItem", "position": 1, "name": "The Local LLM Index", "item": SITE_URL + "/"},
...
<meta property="og:title" content="{esc(it['full_name'])} — The Local LLM Index">
...
<script>var t;try{{t=localStorage.getItem('ll-theme')}}catch(e){{}}if(t)document.documentElement.setAttribute('data-theme',t);</script>
</head>
<body>
<header><div class="wrap head-row">
  <div class="brand disp"><a href="/" style="text-decoration:none;color:inherit">Local&nbsp;LLM&nbsp;Index</a> <span class="tag">/ local</span></div>
  <div class="head-actions"><a href="/">← All tools</a><button class="theme-btn" id="theme" aria-label="Toggle theme">◐</button></div>
</div></header>
<main class="wrap detail">
  <div class="crumb"><a href="/">The Local LLM Index</a> / {esc(it['category'])} / #{it['rank']}</div>
...
<footer><div class="wrap foot-row">
  <div class="blurb">The Local LLM Index is a self-updating spec sheet, recomputed daily from live GitHub signals.</div>
  <div class="links"><a href="/">All tools</a><a href="/rss.xml">RSS</a><a href="/sitemap.xml">Sitemap</a></div>
</div></footer>
<script>document.getElementById('theme').addEventListener('click',function(){{var c=document.documentElement.getAttribute('data-theme')==='dark'?'light':'dark';document.documentElement.setAttribute('data-theme',c);try{{localStorage.setItem('ll-theme',c)}}catch(e){{}}}});</script>
</body>
</html>
"""
