#!/usr/bin/env python3
"""The Local LLM Index — recompute the living index of tools for running LLMs LOCALLY /
on-device from live GitHub signals, and write data.json + SEO (sitemap, rss, robots, llms.txt).

Scope = anything that helps you run a model on your own hardware: inference engines (vLLM,
llama.cpp, MNN, web-llm), runners & apps (Ollama, GPT4All, LM Studio, Jan), local chat UIs,
quantization & GGUF/AWQ tooling, and on-device / mobile model stacks. Gathered across several
GitHub searches, deduped, FILTERED (precision over recall), categorized, and scored by momentum.
...
SITE_URL = "https://local-llm-index.vercel.app"   # fixed to the real alias after first deploy
SITE_NAME = "The Local LLM Index"

QUERIES = [
    "topic:local-llm stars:>10",
    "topic:on-device-ai stars:>5",
    "run llm locally in:name,description stars:>20",
    "local llm in:name,description stars:>60",
    "llm inference engine in:name,description stars:>80",
    "on-device llm in:name,description stars:>20",
    "gguf in:name,description stars:>40",
    "quantization llm in:name,description stars:>60",
    "local ai chat in:name,description stars:>60",
]
...
HEADERS = {"Accept": "application/vnd.github+json", "User-Agent": "local-llm-index"}
if token():
    HEADERS["Authorization"] = f"Bearer {token()}"

_LL_TOPICS = {"local-llm", "on-device-ai", "on-device-llm", "llm-inference", "gguf",
              "quantization", "llamacpp", "ollama", "edge-ai", "local-ai"}
_LL_PHRASES = re.compile(
    r"\b(run[- ]?(llms?|models?) (locally|on[- ]device)|local(ly)? (llm|model|ai)|on[- ]device"
    r"|inference engine|gguf|ggml|quantiz|llama\.?cpp|ollama|offline (llm|ai)|edge (llm|ai)"
    r"|in-?browser (llm|inference))\b", re.I)
_DENY = {"snailclimb/javaguide", "open-multi-agent/open-multi-agent"}
_ANTI = re.compile(
    r"\b(awesome-?list of papers|must-read|reading list|paper list|cloud platform"
    r"|kubernetes|chatgpt api wrapper|prompt collection|sql injection)\b", re.I)
...
def is_local_llm(r: dict) -> bool:
    ...
    if topics & _LL_TOPICS:
        return True
    return bool(_LL_PHRASES.search(f"{r.get('name','')} {desc}"))


def categorize(r: dict) -> str:
    topics = {t.lower() for t in (r.get("topics") or [])}
    blob = f"{(r.get('name') or '').lower()} {(r.get('description') or '').lower()} {' '.join(topics)}"
    if re.search(r"awesome|curated|collection of|directory", blob):
        return "Collections"
    if re.search(r"quantiz|gguf|ggml|awq|exllama|gptq|compression", blob):
        return "Quantization & Formats"
    if re.search(r"mobile|on-?device|android|ios|edge|raspberry", blob):
        return "On-Device & Mobile"
    if re.search(r"in-?browser|web-?llm|wasm|webgpu", blob):
        return "In-Browser"
    if re.search(r"\bui\b|webui|desktop app|chat app|gui|frontend", blob):
        return "Local UIs & Apps"
    if re.search(r"inference engine|serving|throughput|vllm|llama\.?cpp|runtime|server", blob):
        return "Inference Engines"
    if re.search(r"\bollama\b|run |runner|one (file|command)|distribute", blob):
        return "Runners"
...
def build_items() -> list[dict]:
    ...
    for q in QUERIES:
        for r in search(q):
            ...
            if full and full not in seen and is_local_llm(r):
                seen[full] = r
        time.sleep(0.7)
...
def write_seo(data: dict) -> None:
    ...
    open(os.path.join(HERE, "rss.xml"), "w", encoding="utf-8").write(
        ...
        "    <description>The living index of tools for running LLMs locally and on-device.</description>\n"
        + "\n".join(rss_items) + "\n  </channel>\n</rss>\n")

    lines = [f"# {SITE_NAME}", "",
             "> The living index of tools for running LLMs LOCALLY and on-device — inference",
             "> engines, runners, local UIs, quantization — ranked daily by GitHub momentum.", "",
             f"Updated: {data['generated_at']}", f"Tools indexed: {data['count']}", "",
             "## Top local-LLM tools by momentum", ""]
    for it in items[:40]:
...
def main() -> int:
    ...
    items = build_items()
    if not items:
        print("ERROR: no local-LLM tools found — refusing to write empty data.json", file=sys.stderr)
        return 1
    data = write_json(items)
    write_seo(data)
    print(f"wrote data.json: {len(items)} local-LLM tools across {len(data['categories'])} categories")
